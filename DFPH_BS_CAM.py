...
import utils.grad_cam as gradc
...
# def adsh_algo(code_length, tt):
    ...
    #nus
    if d==0:
        lri = {16: 0.003, 32: 0.0012, 48: 0.001, 64: 0.001}
    ...
    scheduler = torch.optim.lr_scheduler.LambdaLR(optimizer_hash, lr_lambda=lambda epoch: (81 - epoch) / (80 + 1))
    ...
    cam_model = models.resnet18(pretrained=True)
    grad_cam = gradc.GradCAM(cam_model, cam_model.layer4[-1], 224)
        ...
        for z in range(20):
            ...
            for epoch in range(epochi):
                ...
                for iter, traindata in enumerate(train_loader, 0):
                    ...
                    cam = grad_cam.calculate_cam(train_img).detach()
                    #print(torch.max(train_img),torch.min(train_img))#tensor(2.6400) tensor(-2.1179)
                    #print(torch.max(cam),torch.min(cam))#tensor(1., device='cuda:0') tensor(0., device='cuda:0'
                    #print(torch.sum(c) for c in cam)
                    # for iii in range(cam.shape[0]):
                    #     print(iii,torch.sum(cam[iii]))
                    # t = train_img.cpu().data.numpy()
                    # t = t.transpose(0,2,3,1)
                    # cv2.imwrite("t0.jpg", t[1]*255)
                    # # cv2.imwrite("t0.jpg", t[1]*255)
                    # # print(t.shape) 40, 224, 224, 3)
                    # t = cam.cpu().data.numpy()
                    # t = t.transpose(0,2,3,1)
                    # cv2.imwrite("cam.jpg", t[1]*255)
                    # #print(cam.shape)

                    # bb
                    train_img = train_img+cam*0.8
